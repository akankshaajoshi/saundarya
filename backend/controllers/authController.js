...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (freshUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError("User password recently changed, please log in again.", 401));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(new AppError("There was an error with sending the email. Try again later!", 500));
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  const user = await User.findById(req.user.id).select("+password"); //From middleware
